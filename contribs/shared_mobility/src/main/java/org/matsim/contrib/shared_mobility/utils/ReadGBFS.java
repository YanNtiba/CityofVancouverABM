...
/**
 * Converts a GBFS feed to a MATSim sharing-service XML (station-based).
 *
 * Example IntelliJ “Program arguments” — all one line:<br>
 * <pre>
 * --url https://vancouver-gbfs.smoove.pro/gbfs/gbfs.json
 * --output-path C:/…/mobi_service.xml
 * --network-path C:/…/mapped-network.xml
...
public final class ReadGBFS {
	...
	public static void main(String[] args) throws IOException, ConfigurationException {
		...
		/* ── 0. accept the funky TLS cert on smoove.pro ─────────── */

		InsecureSsl.trustAllHosts();          //  <-- the only line that changed

		/* ── 1. CRS transform ───────────────────────────────────── */
		CoordinateTransformation tf = new GeotoolsTransformation(
			"EPSG:4326",                  // GBFS = WGS-84
		...
		/* ── 3. auto-discover inner feed URLs ───────────────────── */
		/* ── 3. auto-discover inner feed URLs ───────────────────── */
		Map<String, URL> feeds = new HashMap<>();
		URL discovery = new URL(cmd.getOptionStrict("url"));
		try (BufferedReader r = IOUtils.getBufferedReader(discovery)) {
			feeds = parseFeeds(r);
		} catch (IOException ex) {
			// try language-specific path automatically
			if (!discovery.toString().contains("/en/")) {
				URL fallback = new URL(discovery.toString().replace("/gbfs.json", "/en/gbfs.json"));
		...
		try (BufferedReader r = IOUtils.getBufferedReader(feeds.get("station_information"))) {
			for (JsonNode s : new ObjectMapper().readTree(r).findPath("stations")) {
				Id<SharingStation> sid = Id.create(s.get("station_id").asText(), SharingStation.class);
		...
		/* station_status.json – create one vehicle per *available* bike */
		try (BufferedReader r = IOUtils.getBufferedReader(feeds.get("station_status"))) {
			int vid = 0;
			for (JsonNode s : new ObjectMapper().readTree(r).findPath("stations")) {
				Id<SharingStation> sid = Id.create(s.get("station_id").asText(), SharingStation.class);
		...
		System.out.println("✅  Wrote sharing-service XML with "
			+ spec.getStations().size() + " stations and "
			+ spec.getVehicles().size() + " vehicles.");
	}
	// Place the parseFeeds method here
