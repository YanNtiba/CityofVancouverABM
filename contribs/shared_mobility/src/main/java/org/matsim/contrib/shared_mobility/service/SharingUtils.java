...
public class SharingUtils {
	...
	public static Map<String, Integer> loadCapacitiesFrom(String xmlPath) {
		...
		try {
			...
			for (int i = 0; i < stationNodes.getLength(); i++) {
				...
				if (node.getNodeType() == Node.ELEMENT_NODE) {
					...
					String capStr = elem.getAttribute("capacity");

					if (id != null && capStr != null && !capStr.isEmpty()) {
						capacities.put(id, Integer.parseInt(capStr));
					}
				}
			}

		} catch (Exception e) {
			throw new RuntimeException("Failed to parse capacities from: " + xmlPath, e);
		}
